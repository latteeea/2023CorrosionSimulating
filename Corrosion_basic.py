...
m = float(input("배율: "))
first_k = float(input("K1: "))
Kn = float(input("Kn: "))
del_K = float(input("del_K: "))
...
for ss in range (1, times+1): # if 첫번째 케이스일때랑 그 다음일때 나눠서 써야할듯 (for 문 안에다가 집어넣긴 하지만)
    ...
    list_k = []    # 도달 해야하는 K% 값 리스트        # 면적율 다 끝나면 하나의 케이스 끝난거임
        
    i = 0
    tmp = first_k
    while (tmp <= Kn):
        tmp = first_k + (i * del_K)
        list_k.append(round(tmp,3))
        i += 1
    ...
    while (len(list_k) >= 0):        # 하나의 케이스가 끝날 때까지
        ...
        if (t == 0):
            ...
            for i in range(len(circle)):
                ...
                # print("tmp=", tmp)
                first = round((x0 - b * math.log((a / (circle[i][2])) - 1)), 3)
            ...
            if (len(error) > 0):    #결합
                for i in range(len(error)):
                    a = error[i][0]
                    b = error[i][1]
                    circle[a] = comb(a, b, x0, m, circle[a][2], circle[b][2], circle[a][0], circle[b][0], circle[a][1], circle[b][2], t)
            ...
            # 0 cycle에서 면적율이 충족할 경우
            if (area_ratio(area,circle) > list_k[0]):
                    output[0] = list_k[0]
                    del list_k[0]
            ...
            for j in range(len(circle)):      # j는 circle key
                ...
                circle[j][2] = r_cycle(a,b,x0,thisCycle)     # 사이클에 맞는 반지름 구함
            ...
            if (len(error) >= 1):
                ...
                try: 
                    circle[aa] = comb(a, b, x0, m, circle[aa][2], circle[bb][2], circle[aa][0], circle[bb][0], circle[aa][1], circle[bb][1], t)
            ...
            if (len(list_k) == 0):
                print(ss,"번째 output =", output)
                print("초기면적율 =", areaRatio)
                
                for cu, (key,value) in enumerate(output.items()):
                    print("사이클:",key, end = ", ")
                    print("도달 면적율:", value)

                drawCircle(area, circle)
                print("최종 결함 정보 =", circle)
                print("최종 면적율:", round(area_ratio(area, circle),3))
                break
